    ...
    streamChat(
      {
        message: text,
        use_rag: opts.useRag,
        use_search: opts.useSearch,
        attached_file: opts.attachedFile,
      },
      session,
      (token) =>
        setMessages((prev) => {
      ...
      (meta) => {
        pendingConvId.current = meta.conversation_id;
      },
      () => {
        setStreaming(false);
        if (pendingConvId.current)
          router.replace(`/chat/${pendingConvId.current}`);
      },
      (err) => {
        setStreaming(false);
        setError(err);
  ...
  const handleAnkiCreated = (
    ...
    setMessages((prev) => [
      ...prev,
      {
        ...
        attachedFile: userMessage.attachedFile,
      },
